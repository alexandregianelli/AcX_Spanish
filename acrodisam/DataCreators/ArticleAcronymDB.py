...
from helper import getArticleAcronymDBPath, get_acronym_db_path,\
...
if __name__ == '__main__':
    config_name = "Test=UsersWikipedia:TrainOut=FullWikipedia_MadDog:TrainIn=Ab3P-BioC"
    dbs_file_name = (
            getDatasetGeneratedFilesPath(config_name)
            + "db_"
            + str("TrainData")
            + "_%s.sqlite"
        )
    with SqliteDict(
                dbs_file_name % "TrainAcronyms", flag="r"
            ) as train_acronyms, SqliteDict(dbs_file_name % "TrainArticleAcronyms", flag="n", autocommit=True
            ) as train_article_acronyms:
        create_article_acronym_db_from_acronym_db(train_acronyms, train_article_acronyms)
